...
export const HackingToolkit: React.FC = () => {
  ...
  const tools: Record<ToolType, { name: string; cmd: string; log: string[] }> = {
    METASPLOIT: {
      ...
      cmd: "msfconsole -q -x 'use exploit/windows/smb/ms17_010_eternalblue; set RHOST corporate_spy_node; run'",
      log: [
        "[*] TITAN_OMEGA_V18 Exploit initiated...",
        "[*] Connecting to corporate_spy_node:445...",
        ...
        "[+] Meterpreter session 1 opened (127.0.0.1:4444 -> target:52132)",
        "meterpreter > getuid",
        "Server username: NT AUTHORITY\\SYSTEM",
        "meterpreter > hashdump",
        "[+] Administrator:500:aad3b435b51404eeaad3b435b51404ee:31d6cfe0d16ae931b73c59d7e0c089c0:::",
        "[*] Extracting hidden tracking logs for legal strike evidence..."
      ]
    },
    SQLMAP: {
      name: "SQLMap Deep Extractor",
      cmd: "sqlmap -u 'http://corporate-hypocrisy.sa/api/v1/tracking?id=1' --dump --risk 3",
      log: [
        ...
        "    [*] admin_credentials",
        "[*] Dumping table 'third_party_chinese_exfiltration'...",
        "[+] Found 1.2 Million records. Exporting to /tmp/sovereign_evidence.csv"
      ]
    },
    NJ_RAT: {
      name: "NJ-RAT Command Center",
      cmd: "njrat --target scammer_node --capture-desktop --reclaim-assets --establish-p2p",
      log: [
        "[*] INITIALIZING PEER-TO-PEER UPLINK THROUGH ENCRYPTED MESH...",
        "[+] P2P LINK VERIFIED: SCAMMER_NODE_0x403 (Uplink Stable)",
        ...
        "[*] MONITORING ACTIVE SESSIONS... [Banking_Vault_App.exe detected]",
        "[*] ANALYZING TRANSACTION_HISTORY for fraud patterns...",
        "[+] FRAUD DETECTED: Illegal exfiltration of user assets confirmed.",
        "[*] INITIATING ETHICAL RECLAMATION_PROTOCOL_V18...",
        "[*] ATTEMPTING SECURE FUND REDIRECTION TO SOVEREIGN_VAULT...",
        "[+] RECLAMATION SUCCESSFUL: 42.8 ETH recovered and secured.",
        "[*] DEPLOYING STEALTH DISCONNECT... [NO_LOGS_REMAINING]",
        "[*] SESSION CLOSED."
      ]
    },
    SOVEREIGN_BYPASS: {
    ...
    INFRA_AUDIT: {
      ...
      cmd: "nmap -sV -sC -Pn target_corporate_infrastructure",
      log: [
        "Starting Nmap 9.0 ( https://nmap.org ) at 2026-02-14 04:03 UTC",
        "Nmap scan report for node.corporate-spyware.com (10.0.0.1)",
  ...
  return (
    <div className="h-full flex flex-col gap-8 animate-in slide-in-from-bottom-10 duration-700">
      ...
      <div className="flex-1 bt4-glass border-4 border-purple-900/40 p-12 font-mono bg-black relative shadow-[inset_0_0_100px_rgba(168,85,247,0.1)] overflow-hidden">
        ...
        <div ref={scrollRef} className="space-y-3 h-[450px] overflow-y-auto scrollbar-hide text-[12px] leading-relaxed">
          {output.map((line, i) => {
            ...
            const isWarning = line.includes('[!]') || line.includes('VULNERABLE');
            ...
            return (
              <div key={i} className={`flex gap-8 ${isSuccess ? 'text-green-400 font-bold' : isWarning ? 'text-red-500 font-black' : isInfo ? 'text-cyan-400' : 'text-zinc-500'}`}>
                 ...
                 <span className="break-all italic">{line}</span>
              </div>
            );
          })}
