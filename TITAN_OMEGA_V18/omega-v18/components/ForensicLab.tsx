...
export const ForensicLab: React.FC<ForensicLabProps> = ({ files, onViewDetails }) => {
  ...
  return (
    <div className="flex-1 space-y-10">
      {files.map((file, idx) => {
        const safeMime = String(file?.mime || 'application/octet-stream');
        const checkMime = (val: string) => typeof safeMime.includes === 'function' ? safeMime.includes(val) : false;
          ...
          <div key={idx} className="bt4-glass border-l-[20px] border-red-600 p-12 bg-black/80 animate-in slide-in-from-right duration-700 shadow-3xl group relative">
            <button 
              onClick={() => onViewDetails({
                id: `ev-${idx}-${Date.now()}`,
                title: file.name || 'UNKNOWN_ASSET',
                type: checkMime('code') ? 'CODE' : 'LOG',
                severity: 'CRITICAL',
            ...
            <div className="flex justify-between items-start mb-10 pr-20">
              <div className="flex items-center gap-10">
                ...
                <div>
                  ...
                  <p className="text-[13px] text-zinc-500 font-mono mt-3 tracking-widest uppercase font-black">{safeMime} // SHA256_{Math.random().toString(16).slice(2, 14).toUpperCase()}</p>
                </div>
              </div>
