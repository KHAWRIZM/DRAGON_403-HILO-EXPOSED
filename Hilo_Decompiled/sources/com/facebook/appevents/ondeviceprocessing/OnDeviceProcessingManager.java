...
public final class OnDeviceProcessingManager {
    ...
    private final boolean isEventEligibleForOnDeviceProcessing(AppEvent event) {
        ...
        try {
            if (event.isImplicit() && ALLOWED_IMPLICIT_EVENTS.contains(event.getName())) {
                z10 = true;
            } else {
                z10 = false;
            }
            ...
            if (INSTANCE.isEventEligibleForOnDeviceProcessing(event)) {
                FacebookSdk.getExecutor().execute(new Runnable() { // from class: com.facebook.appevents.ondeviceprocessing.b
                    ...
                    public final void run() {
                        OnDeviceProcessingManager.m٣٤٥sendCustomEventAsync$lambda1(applicationId, event);
                    }
                });
            }
        } catch (Throwable th) {
    ...
    public static final void m٣٤٥sendCustomEventAsync$lambda1(String applicationId, AppEvent event) {
        if (CrashShieldHandler.isObjectCrashing(OnDeviceProcessingManager.class)) {
            return;
        }
        try {
        ...
        try {
            ...
            if (applicationContext != null && applicationId != null && preferencesName != null) {
                FacebookSdk.getExecutor().execute(new Runnable() { // from class: com.facebook.appevents.ondeviceprocessing.a
                    @Override // java.lang.Runnable
                    public final void run() {
                        OnDeviceProcessingManager.m٣٤٦sendInstallEventAsync$lambda0(applicationContext, preferencesName, applicationId);
                    }
                });
            }
        } catch (Throwable th) {
    ...
    public static final void m٣٤٦sendInstallEventAsync$lambda0(Context context, String str, String str2) {
        if (CrashShieldHandler.isObjectCrashing(OnDeviceProcessingManager.class)) {
            return;
        }
        try {
