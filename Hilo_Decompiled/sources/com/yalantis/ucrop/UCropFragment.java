...
public class UCropFragment extends Fragment {
    ...
    private void changeSelectedTab(int i) {
        ...
        int i3;
        if (getView() != null) {
            x.b((ViewGroup) getView().findViewById(R.id.ucrop_photobox), this.mControlsTransition);
        }
    ...
    private void processOptions(Bundle bundle) {
        ...
        overlayView.setDimmedColor(bundle.getInt(UCrop.Options.EXTRA_DIMMED_LAYER_COLOR, resources.getColor(i)));
        this.mOverlayView.setCircleStrokeColor(bundle.getInt(UCrop.Options.EXTRA_CIRCLE_STROKE_COLOR, getResources().getColor(i)));
        ...
        overlayView2.setCropGridStrokeWidth(bundle.getInt(UCrop.Options.EXTRA_CROP_GRID_STROKE_WIDTH, resources2.getDimensionPixelSize(i2)));
        this.mOverlayView.setDimmedStrokeWidth(bundle.getInt(UCrop.Options.EXTRA_CIRCLE_STROKE_WIDTH_LAYER, getResources().getDimensionPixelSize(i2)));
        ...
        if (f >= 0.0f && f2 >= 0.0f) {
            ...
            float aspectRatioX = ((AspectRatio) parcelableArrayList.get(i3)).getAspectRatioX() / ((AspectRatio) parcelableArrayList.get(i3)).getAspectRatioY();
        ...
        int i4 = bundle.getInt(UCrop.EXTRA_MAX_SIZE_X, 0);
        int i5 = bundle.getInt(UCrop.EXTRA_MAX_SIZE_Y, 0);
        if (i4 > 0 && i5 > 0) {
    ...
    private void setupRotateWidget(View view) {
        ...
        ((HorizontalProgressWheelView) view.findViewById(i)).setMiddleLineColor(this.mActiveControlsWidgetColor);
    ...
    protected UCropResult getResult(Uri uri, float f, int i, int i2, int i3, int i4) {
        return new UCropResult(-1, new Intent().putExtra(UCrop.EXTRA_OUTPUT_URI, uri).putExtra(UCrop.EXTRA_OUTPUT_CROP_ASPECT_RATIO, f).putExtra(UCrop.EXTRA_OUTPUT_IMAGE_WIDTH, i3).putExtra(UCrop.EXTRA_OUTPUT_IMAGE_HEIGHT, i4).putExtra(UCrop.EXTRA_OUTPUT_OFFSET_X, i).putExtra(UCrop.EXTRA_OUTPUT_OFFSET_Y, i2).putExtra(UCrop.EXTRA_CROP_INPUT_ORIGINAL, FileUtils.getInputPath((Uri) getArguments().getParcelable(UCrop.EXTRA_INPUT_URI))));
    }
    ...
    public void onAttach(Context context) {
        ...
        if (getParentFragment() instanceof UCropFragmentCallback) {
            this.callback = getParentFragment();
        } else {
    ...
    public void setupViews(View view, Bundle bundle) {
        this.mActiveControlsWidgetColor = bundle.getInt(UCrop.Options.EXTRA_UCROP_COLOR_CONTROLS_WIDGET_ACTIVE, a.getColor(getContext(), R.color.ucrop_color_active_controls_color));
        this.mLogoColor = bundle.getInt(UCrop.Options.EXTRA_UCROP_LOGO_COLOR, a.getColor(getContext(), R.color.ucrop_color_default_logo));
        this.mShowBottomControls = !bundle.getBoolean(UCrop.Options.EXTRA_HIDE_BOTTOM_CONTROLS, false);
        this.mRootViewBackgroundColor = bundle.getInt(UCrop.Options.EXTRA_UCROP_ROOT_VIEW_BACKGROUND_COLOR, a.getColor(getContext(), R.color.ucrop_color_crop_background));
