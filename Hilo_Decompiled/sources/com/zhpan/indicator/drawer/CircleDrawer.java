...
public final class CircleDrawer extends BaseDrawer {
    ...
    private final void drawNormal(Canvas canvas) {
        ...
        for (int i = 0; i < pageSize; i++) {
            IndicatorUtils indicatorUtils = IndicatorUtils.INSTANCE;
            drawCircle(canvas, indicatorUtils.getCoordinateX(getMIndicatorOptions(), getMaxWidth(), i), indicatorUtils.getCoordinateY(getMaxWidth()), normalSliderWidth / 2);
        }
    }
    ...
    private final void drawScaleSlider(Canvas canvas) {
        ...
        if (currentPosition == getMIndicatorOptions().getPageSize() - 1) {
            ArgbEvaluator argbEvaluator2 = getArgbEvaluator();
            if (argbEvaluator2 != null) {
                obj = argbEvaluator2.evaluate(slideProgress, Integer.valueOf(getMIndicatorOptions().getNormalSliderColor()), Integer.valueOf(getMIndicatorOptions().getCheckedSliderColor()));
            }
            Paint mPaint2 = getMPaint();
            if (obj != null) {
                mPaint2.setColor(((Integer) obj).intValue());
                float f2 = 2;
        ...
        if (slideProgress > 0) {
            ...
            if (argbEvaluator3 != null) {
                obj = argbEvaluator3.evaluate(slideProgress, Integer.valueOf(getMIndicatorOptions().getNormalSliderColor()), Integer.valueOf(getMIndicatorOptions().getCheckedSliderColor()));
            }
            Paint mPaint3 = getMPaint();
            if (obj != null) {
                mPaint3.setColor(((Integer) obj).intValue());
                float f3 = 2;
    ...
    private final void drawWormSlider(Canvas canvas) {
        ...
        float f = 2;
        float coerceAtLeast = (RangesKt.coerceAtLeast(((slideProgress - 0.5f) * sliderGap) * 2.0f, 0.0f) + coordinateX) - (getMIndicatorOptions().getNormalSliderWidth() / f);
        float f2 = 3;
        this.rectF.set(coerceAtLeast + f2, f2, coordinateX + RangesKt.coerceAtMost(slideProgress * sliderGap * 2.0f, sliderGap) + (getMIndicatorOptions().getNormalSliderWidth() / f) + f2, normalSliderWidth + f2);
