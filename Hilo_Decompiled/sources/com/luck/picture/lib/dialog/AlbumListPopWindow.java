...
public class AlbumListPopWindow extends PopupWindow {
    ...
    public void changeSelectedAlbumStyle() {
        ...
        for (int i10 = 0; i10 < albumList.size(); i10++) {
            ...
            for (int i11 = 0; i11 < this.selectorConfig.getSelectCount(); i11++) {
                if (TextUtils.equals(localMediaFolder.getFolderName(), this.selectorConfig.getSelectedResult().get(i11).getParentFolderName()) || localMediaFolder.getBucketId() == -1) {
    ...
    public void dismiss() {
        ...
        this.windMask.setAlpha(DownloadProgress.UNKNOWN_PROGRESS);
    ...
    public LocalMediaFolder getFolder(int i10) {
        if (this.mAdapter.getAlbumList().size() > 0 && i10 < this.mAdapter.getAlbumList().size()) {
            return this.mAdapter.getAlbumList().get(i10);
        }
        return null;
    }
