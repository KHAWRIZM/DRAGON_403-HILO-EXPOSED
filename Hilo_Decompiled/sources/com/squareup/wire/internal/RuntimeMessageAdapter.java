...
public final class RuntimeMessageAdapter<M, B> extends ProtoAdapter<M> {
    ...
    public RuntimeMessageAdapter(@NotNull MessageBinding<M, B> messageBinding) {
    ...
    public final String getJsonName(@NotNull FieldOrOneOfBinding<?, ?> fieldOrOneOfBinding) {
        ...
        if (fieldOrOneOfBinding.getWireFieldJsonName().length() == 0) {
            return fieldOrOneOfBinding.getDeclaredName();
        }
        return fieldOrOneOfBinding.getWireFieldJsonName();
    }
        ...
        for (FieldOrOneOfBinding<M, B> fieldOrOneOfBinding : this.fields.values()) {
            ...
            if (!fieldOrOneOfBinding.getRedacted() && (!isMessage || fieldOrOneOfBinding.getLabel().isRepeated())) {
                if (isMessage && fieldOrOneOfBinding.getLabel().isRepeated()) {
                    ...
                    if (fromBuilder != null) {
                        fieldOrOneOfBinding.set(newBuilder, Internal.m١١٧redactElements((List) fromBuilder, fieldOrOneOfBinding.getSingleAdapter()));
                    } else {
