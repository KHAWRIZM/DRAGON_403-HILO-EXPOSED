...
public class AWS4Signer extends AbstractAWSSigner implements ServiceAwareSigner, RegionAwareSigner, Presigner {
    ...
    protected final String getDateStamp(long j10) {
        return DateUtils.format(DATE_PATTERN, new Date(j10));
    }
    ...
    protected final String getTimeStamp(long j10) {
        return DateUtils.format("yyyyMMdd'T'HHmmss'Z'", new Date(j10));
    }

    boolean needsSign(String str) {
        if (!"date".equalsIgnoreCase(str) && !Headers.CONTENT_MD5.equalsIgnoreCase(str) && !"host".equalsIgnoreCase(str) && !str.startsWith("x-amz") && !str.startsWith("X-Amz")) {
            return false;
        }
        return true;
    }
    ...
    public void presignRequest(Request<?> request, AWSCredentials aWSCredentials, Date date) {
        ...
        if (date != null) {
            j10 = (date.getTime() - System.currentTimeMillis()) / MILLISEC;
        } else {
            j10 = 604800;
        }
        if (j10 <= MAX_EXPIRATION_TIME_IN_SECONDS) {
            ...
            request.addParameter("X-Amz-Expires", Long.toString(j10));
