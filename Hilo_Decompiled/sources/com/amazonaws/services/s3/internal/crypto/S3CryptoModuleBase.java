...
public abstract class S3CryptoModuleBase<T extends MultipartUploadCryptoContext> extends S3CryptoModule<T> {
    ...
    protected final S3Direct f٦٩٢٩s3;
    ...
    private CipherLiteInputStream newS3CipherLiteInputStream(AbstractPutObjectRequest abstractPutObjectRequest, ContentCryptoMaterial contentCryptoMaterial, long j10) {
    ...
    private PutObjectResult putObjectUsingInstructionFile(PutObjectRequest putObjectRequest) {
        ...
        PutObjectRequest withInputStream = putObjectRequest.mo٢٦٩clone().withFile((File) null).withInputStream((InputStream) null);
        ...
        try {
            PutObjectResult putObject = this.f٦٩٢٩s3.putObject(putObjectRequest2);
            S3DataSource.Utils.cleanupDataSource(putObjectRequest, file, inputStream, putObjectRequest2.getInputStream(), this.log);
            this.f٦٩٢٩s3.putObject(updateInstructionPutRequest(withInputStream, createContentCryptoMaterial));
            return putObject;
        } catch (Throwable th) {
            S3DataSource.Utils.cleanupDataSource(putObjectRequest, file, inputStream, putObjectRequest2.getInputStream(), this.log);
            throw th;
    ...
    private PutObjectResult putObjectUsingMetadata(PutObjectRequest putObjectRequest) {
        ...
        try {
            return this.f٦٩٢٩s3.putObject(putObjectRequest2);
        } finally {
            S3DataSource.Utils.cleanupDataSource(putObjectRequest, file, inputStream, putObjectRequest2.getInputStream(), this.log);
        }
    }
    ...
    public final void abortMultipartUploadSecurely(AbortMultipartUploadRequest abortMultipartUploadRequest) {
        this.f٦٩٢٩s3.abortMultipartUpload(abortMultipartUploadRequest);
        this.multipartUploadContexts.remove(abortMultipartUploadRequest.getUploadId());
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public final <X extends AmazonWebServiceRequest> X appendUserAgent(X x10, String str) {
        x10.getRequestClientOptions().appendUserAgent(str);
        return x10;
    }

    abstract CipherLite cipherLiteForNextPart(T t10);
    ...
    public CompleteMultipartUploadResult completeMultipartUploadSecurely(CompleteMultipartUploadRequest completeMultipartUploadRequest) {
        ...
        T t10 = this.multipartUploadContexts.get(uploadId);
        if (t10 != null && !t10.hasFinalPartBeenSeen()) {
        ...
        CompleteMultipartUploadResult completeMultipartUpload = this.f٦٩٢٩s3.completeMultipartUpload(completeMultipartUploadRequest);
        if (t10 != null && this.cryptoConfig.getStorageMode() == CryptoStorageMode.InstructionFile) {
            this.f٦٩٢٩s3.putObject(createInstructionPutRequest(t10.getBucketName(), t10.getKey(), t10.getContentCryptoMaterial()));
        }
    ...
    public final CopyPartResult copyPartSecurely(CopyPartRequest copyPartRequest) {
        T t10 = this.multipartUploadContexts.get(copyPartRequest.getUploadId());
        CopyPartResult copyPart = this.f٦٩٢٩s3.copyPart(copyPartRequest);
        if (t10 != null && !t10.hasFinalPartBeenSeen()) {
            t10.setHasFinalPartBeenSeen(true);
        }
        return copyPart;
    }
    ...
    public final S3ObjectWrapper fetchInstructionFile(S3ObjectId s3ObjectId, String str) {
        try {
            S3Object object = this.f٦٩٢٩s3.getObject(createInstructionGetRequest(s3ObjectId, str));
            if (object == null) {
                return null;
            }
            ...
            if (this.log.isDebugEnabled()) {
                this.log.debug("Unable to retrieve instruction file : " + e10.getMessage());
            }
            return null;
        }
    }
    ...
    public InitiateMultipartUploadResult initiateMultipartUploadSecurely(InitiateMultipartUploadRequest initiateMultipartUploadRequest) {
        ...
        InitiateMultipartUploadResult initiateMultipartUpload = this.f٦٩٢٩s3.initiateMultipartUpload(initiateMultipartUploadRequest);
    ...
    public final PutObjectResult putInstructionFileSecurely(PutInstructionFileRequest putInstructionFileRequest) {
        ...
        S3Object object = this.f٦٩٢٩s3.getObject(getObjectRequest);
        ...
        if (object != null) {
            ...
            try {
                ...
                return this.f٦٩٢٩s3.putObject(updateInstructionPutRequest(putInstructionFileRequest.createPutObjectRequest(object), recreate));
            } catch (Error e10) {
                IOUtils.closeQuietly(object, this.log);
                throw e10;
    ...
    public final void putLocalObjectSecurely(UploadObjectRequest uploadObjectRequest, String str, OutputStream outputStream) throws IOException {
        UploadObjectRequest uploadObjectRequest2 = uploadObjectRequest.mo٢٦٩clone();
        File file = uploadObjectRequest2.getFile();
        InputStream inputStream = uploadObjectRequest2.getInputStream();
        T t10 = this.multipartUploadContexts.get(str);
        UploadObjectRequest uploadObjectRequest3 = (UploadObjectRequest) wrapWithCipher(uploadObjectRequest2, t10.getContentCryptoMaterial());
        try {
            IOUtils.copy(uploadObjectRequest3.getInputStream(), outputStream);
            t10.setHasFinalPartBeenSeen(true);
        } finally {
            S3DataSource.Utils.cleanupDataSource(uploadObjectRequest3, file, inputStream, uploadObjectRequest3.getInputStream(), this.log);
    ...
    abstract void updateUploadContext(T t10, SdkFilterInputStream sdkFilterInputStream);
    ...
    public UploadPartResult uploadPartSecurely(UploadPartRequest uploadPartRequest) {
        ...
        T t10 = this.multipartUploadContexts.get(uploadId);
        if (t10 != null) {
            t10.beginPartUpload(uploadPartRequest.getPartNumber());
            CipherLite cipherLiteForNextPart = cipherLiteForNextPart(t10);
            File file = uploadPartRequest.getFile();
            InputStream inputStream = uploadPartRequest.getInputStream();
            ...
            try {
                ...
                try {
                    ...
                    UploadPartResult uploadPart = this.f٦٩٢٩s3.uploadPart(uploadPartRequest);
                    S3DataSource.Utils.cleanupDataSource(uploadPartRequest, file, inputStream, wrapForMultipart, this.log);
                    t10.endPartUpload();
                    if (isLastPart) {
                        t10.setHasFinalPartBeenSeen(true);
                    }
                    updateUploadContext(t10, wrapForMultipart);
                    return uploadPart;
                } catch (Throwable th) {
                    th = th;
    ...
    abstract <I extends CipherLiteInputStream> SdkFilterInputStream wrapForMultipart(I i10, long j10);

    protected final <R extends AbstractPutObjectRequest> R wrapWithCipher(R r10, ContentCryptoMaterial contentCryptoMaterial) {
        ObjectMetadata metadata = r10.getMetadata();
        if (metadata == null) {
            metadata = new ObjectMetadata();
        }
        ...
        long plaintextLength = plaintextLength(r10, metadata);
        ...
        r10.setMetadata(metadata);
        r10.setInputStream(newS3CipherLiteInputStream(r10, contentCryptoMaterial, plaintextLength));
        r10.setFile(null);
        return r10;
    }
