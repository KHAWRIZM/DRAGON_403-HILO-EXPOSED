...
public abstract class AbstractPutObjectRequest extends AmazonWebServiceRequest implements SSECustomerKeyProvider, SSEAwsKeyManagementParamsProvider, S3DataSource, Serializable {
    ...
    public final <T extends AbstractPutObjectRequest> T copyPutObjectBaseTo(T t10) {
        ObjectMetadata objectMetadata;
        copyBaseTo(t10);
        ObjectMetadata metadata = getMetadata();
        AbstractPutObjectRequest withInputStream = t10.withAccessControlList(getAccessControlList()).withCannedAcl(getCannedAcl()).withInputStream(getInputStream());
        if (metadata == null) {
            objectMetadata = null;
        } else {
            objectMetadata = metadata.m٢٦٧clone();
        }
        return (T) withInputStream.withMetadata(objectMetadata).withRedirectLocation(getRedirectLocation()).withStorageClass(getStorageClass()).withSSEAwsKeyManagementParams(getSSEAwsKeyManagementParams()).withSSECustomerKey(getSSECustomerKey());
    }
    ...
    /* renamed from: clone */
    public AbstractPutObjectRequest mo٢٦٩clone() {
        return (AbstractPutObjectRequest) super.mo٢٦٩clone();
    }
