...
public class BucketConfigurationXmlFactory {
    ...
    private boolean hasCurrentExpirationPolicy(BucketLifecycleConfiguration.Rule rule) {
        if (rule.getExpirationInDays() == -1 && rule.getExpirationDate() == null && !rule.isExpiredObjectDeleteMarker()) {
            return false;
        }
        return true;
    }
    ...
    private boolean hasTags(TagSet tagSet) {
        if (tagSet != null && tagSet.getAllTags() != null && tagSet.getAllTags().size() > 0) {
            return true;
        }
        return false;
    }
