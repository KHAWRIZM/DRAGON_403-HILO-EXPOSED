...
public class UploadObjectObserver {
    ...
    private AmazonS3 f٦٩٢٧s3;
    private S3DirectSpi s3direct;
    private String uploadId;

    protected <X extends AmazonWebServiceRequest> X appendUserAgent(X x10, String str) {
    ...
    public void onAbort() {
        ...
        if (this.uploadId != null) {
            try {
                this.f٦٩٢٧s3.abortMultipartUpload(new AbortMultipartUploadRequest(this.req.getBucketName(), this.req.getKey(), this.uploadId));
            } catch (Exception e10) {
                LogFactory.getLog(getClass()).debug("Failed to abort multi-part upload: " + this.uploadId, e10);
            }
        }
    }

    public CompleteMultipartUploadResult onCompletion(List<PartETag> list) {
        return this.f٦٩٢٧s3.completeMultipartUpload(new CompleteMultipartUploadRequest(this.req.getBucketName(), this.req.getKey(), this.uploadId, list));
    }
    ...
    public String onUploadInitiation(UploadObjectRequest uploadObjectRequest) {
        String uploadId = this.f٦٩٢٧s3.initiateMultipartUpload(newInitiateMultipartUploadRequest(uploadObjectRequest)).getUploadId();
