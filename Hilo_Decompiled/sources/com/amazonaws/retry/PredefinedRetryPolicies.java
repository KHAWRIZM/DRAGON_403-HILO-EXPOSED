...
public class PredefinedRetryPolicies {
    ...
    private static final class SDKDefaultBackoffStrategy implements RetryPolicy.BackoffStrategy {
        ...
        public final long delayBeforeNextRetry(AmazonWebServiceRequest amazonWebServiceRequest, AmazonClientException amazonClientException, int i10) {
            if (i10 <= 0) {
                return 0L;
            }
            return this.random.nextInt(Math.min(this.maxDelayMs, (1 << i10) * this.baseDelayMs));
        }

        private SDKDefaultBackoffStrategy(int i10, int i11) {
            this.random = new Random();
            this.baseDelayMs = i10;
            this.maxDelayMs = i11;
        }
    }
    ...
    public static RetryPolicy getDefaultRetryPolicyWithCustomMaxRetries(int i10) {
        return new RetryPolicy(DEFAULT_RETRY_CONDITION, DEFAULT_BACKOFF_STRATEGY, i10, false);
    }
    ...
    public static RetryPolicy getDynamoDBDefaultRetryPolicyWithCustomMaxRetries(int i10) {
        return new RetryPolicy(DEFAULT_RETRY_CONDITION, DEFAULT_BACKOFF_STRATEGY, i10, false);
    }
}
