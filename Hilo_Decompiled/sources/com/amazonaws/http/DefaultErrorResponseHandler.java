    ...
    public AmazonServiceException handle(HttpResponse httpResponse) throws Exception {
        try {
            ...
            try {
                ...
                return newAmazonServiceException(String.format("Unable to unmarshall error response (%s)", iOUtils), httpResponse, e10);
            }
        } catch (IOException e11) {
            Log log2 = log;
            if (log2.isDebugEnabled()) {
                log2.debug("Failed in reading the error response", e11);
            }
            return newAmazonServiceException("Unable to unmarshall error response", httpResponse, e11);
        }
    }
}
