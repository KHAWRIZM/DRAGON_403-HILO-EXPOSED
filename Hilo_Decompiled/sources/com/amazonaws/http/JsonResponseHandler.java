...
public class JsonResponseHandler<T> implements HttpResponseHandler<AmazonWebServiceResponse<T>> {
    ...
    public AmazonWebServiceResponse<T> handle(HttpResponse httpResponse) throws Exception {
        ...
        if (rawContent == null) {
            rawContent = new ByteArrayInputStream(ServiceProvider__TheRouter__851042640.FLOW_TASK_JSON.getBytes(StringUtils.UTF8));
        }
        log2.debug("CRC32Checksum = " + str);
        log2.debug("content encoding = " + httpResponse.getHeaders().get("Content-Encoding"));
        boolean equals = HttpConstant.GZIP.equals(httpResponse.getHeaders().get("Content-Encoding"));
        if (str != null) {
        ...
        try {
            ...
            if (!this.needsConnectionLeftOpen) {
                try {
                    ...
                    log.warn("Error closing json parser", e10);
                }
            }
            ...
            if (!this.needsConnectionLeftOpen) {
                try {
                    ...
                    log.warn("Error closing json parser", e11);
                }
            }
            throw th;
        }
