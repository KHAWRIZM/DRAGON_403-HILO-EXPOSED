...
public class UploadTask implements Callable<Boolean> {
    ...
    private final AmazonS3 f٦٩٢٦s3;
    ...
    public class UploadTaskProgressListener implements ProgressListener {
        ...
        public synchronized void onProgressChanged(int i10, long j10) {
            UploadPartTaskMetadata uploadPartTaskMetadata = UploadTask.this.uploadPartTasks.get(Integer.valueOf(i10));
            if (uploadPartTaskMetadata == null) {
                UploadTask.LOGGER.info("Update received for unknown part. Ignoring.");
                return;
            }
            uploadPartTaskMetadata.bytesTransferredSoFar = j10;
            Iterator<Map.Entry<Integer, UploadPartTaskMetadata>> it = UploadTask.this.uploadPartTasks.entrySet().iterator();
            long j11 = 0;
            while (it.hasNext()) {
                j11 += it.next().getValue().bytesTransferredSoFar;
            }
            if (j11 > this.prevTotalBytesTransferredOfAllParts) {
                UploadTask.this.updater.updateProgress(UploadTask.this.upload.id, j11, UploadTask.this.upload.bytesTotal, true);
    ...
    public UploadTask(TransferRecord transferRecord, AmazonS3 amazonS3, TransferDBUtil transferDBUtil, TransferStatusUpdater transferStatusUpdater) {
        ...
        this.f٦٩٢٦s3 = amazonS3;
        this.dbUtil = transferDBUtil;
        this.updater = transferStatusUpdater;
    }

    private void abortMultiPartUpload(int i10, String str, String str2, String str3) {
        Log log = LOGGER;
        log.info("Aborting the multipart since complete multipart failed.");
        try {
            this.f٦٩٢٦s3.abortMultipartUpload(new AbortMultipartUploadRequest(str, str2, str3));
            log.debug("Successfully aborted multipart upload: " + i10);
        } catch (AmazonClientException e10) {
            LOGGER.debug("Failed to abort the multipart upload: " + i10, e10);
        }
    }

    private void completeMultiPartUpload(int i10, String str, String str2, String str3) throws AmazonClientException, AmazonServiceException {
        CompleteMultipartUploadRequest completeMultipartUploadRequest = new CompleteMultipartUploadRequest(str, str2, str3, this.dbUtil.queryPartETagsOfUpload(i10));
        TransferUtility.appendMultipartTransferServiceUserAgentString(completeMultipartUploadRequest);
        this.f٦٩٢٦s3.completeMultipartUpload(completeMultipartUploadRequest);
    }
        ...
        if (map != null) {
            ...
            if (str8 != null) {
                try {
                    ...
                    LOGGER.error("Error in passing the object tags as request headers.", e10);
                }
            }
    ...
    private String initiateMultipartUpload(PutObjectRequest putObjectRequest) {
        ...
        return this.f٦٩٢٦s3.initiateMultipartUpload(withTagging).getUploadId();
    }
    ...
    private Boolean uploadMultipartAndWaitForCompletion() throws ExecutionException {
        ...
        if (str != null && !str.isEmpty()) {
            ...
            try {
                ...
                LOGGER.error("Error initiating multipart upload: " + this.upload.id + " due to " + e10.getMessage(), e10);
                this.updater.throwError(this.upload.id, e10);
        ...
        transferStatusUpdater.updateProgress(transferRecord2.id, j10, transferRecord2.bytesTotal, false);
        ...
        for (UploadPartRequest uploadPartRequest : this.requestList) {
            ...
            uploadPartTaskMetadata.uploadPartTask = TransferThreadPool.submitTask(new UploadPartTask(uploadPartTaskMetadata, uploadTaskProgressListener, uploadPartRequest, this.f٦٩٢٦s3, this.dbUtil));
        }
        try {
            Iterator<UploadPartTaskMetadata> it = this.uploadPartTasks.values().iterator();
            boolean z10 = true;
            while (it.hasNext()) {
                z10 &= it.next().uploadPartTask.get().booleanValue();
            }
            if (!z10) {
                try {
            ...
            try {
                ...
                int i10 = transferRecord5.id;
                long j11 = transferRecord5.bytesTotal;
                transferStatusUpdater2.updateProgress(i10, j11, j11, true);
                this.updater.updateState(this.upload.id, TransferState.COMPLETED);
                return Boolean.TRUE;
            } catch (AmazonClientException e12) {
                LOGGER.error("Failed to complete multipart: " + this.upload.id + " due to " + e12.getMessage(), e12);
            ...
            LOGGER.error("Upload resulted in an exception. " + e13);
            ...
            if (TransferState.PENDING_CANCEL.equals(this.upload.state)) {
                ...
                transferStatusUpdater3.updateState(i11, transferState);
            ...
            if (TransferState.PENDING_PAUSE.equals(this.upload.state)) {
                ...
                int i12 = this.upload.id;
                TransferState transferState2 = TransferState.PAUSED;
                transferStatusUpdater4.updateState(i12, transferState2);
            ...
            if (RetryUtils.isInterrupted(e13)) {
                LOGGER.info("Transfer is interrupted. " + e13);
                this.updater.updateState(this.upload.id, TransferState.FAILED);
                return Boolean.FALSE;
            }
            LOGGER.error("Error encountered during multi-part upload: " + this.upload.id + " due to " + e13.getMessage(), e13);
            this.updater.throwError(this.upload.id, e13);
            this.updater.updateState(this.upload.id, TransferState.FAILED);
            return Boolean.FALSE;
        }
    }
    ...
    private Boolean uploadSinglePartAndWaitForCompletion() {
        ...
        try {
            this.f٦٩٢٦s3.putObject(createPutObjectRequest);
            ...
            LOGGER.debug("Failed to upload: " + this.upload.id + " due to " + e10.getMessage());
            this.updater.throwError(this.upload.id, e10);
            this.updater.updateState(this.upload.id, TransferState.FAILED);
            return Boolean.FALSE;
        }
    ...
    public Boolean call() throws Exception {
        ...
        TransferRecord transferRecord = this.upload;
        int i10 = transferRecord.isMultipart;
        if (i10 == 1 && transferRecord.partNumber == 0) {
