        ...
        synchronized (this) {
            ...
            try {
                ...
                while (cursor.moveToNext()) {
                    int i11 = cursor.getInt(cursor.getColumnIndexOrThrow(TransferTable.COLUMN_ID));
                    if (this.updater.getTransfer(i11) == null) {
                ...
                try {
                    ...
                    LOGGER.error("Error in resuming the transfers." + e10.getMessage());
                }
                LOGGER.debug(i10 + " transfers are loaded from database.");
            } catch (Throwable th) {
                if (cursor != null) {
                    LOGGER.debug("Closing the cursor for resumeAllTransfers");
                    cursor.close();
