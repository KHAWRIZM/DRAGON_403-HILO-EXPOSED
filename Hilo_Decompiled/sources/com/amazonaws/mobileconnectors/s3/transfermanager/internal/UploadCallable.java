...
public class UploadCallable implements Callable<UploadResult> {
    ...
    private final AmazonS3 f٦٩١٩s3;
    ...
    public UploadCallable(TransferManager transferManager, ExecutorService executorService, UploadImpl uploadImpl, PutObjectRequest putObjectRequest, ProgressListenerChain progressListenerChain, String str, TransferProgress transferProgress) {
        this.f٦٩١٩s3 = transferManager.getAmazonS3Client();
    ...
    private long getOptimalPartSize(boolean z10) {
    ...
    private Map<Integer, PartSummary> identifyExistingPartsForResume(String str) {
        ...
        int i10 = 0;
        while (true) {
            PartListing listParts = this.f٦٩١٩s3.listParts(new ListPartsRequest(this.putObjectRequest.getBucketName(), this.putObjectRequest.getKey(), str).withPartNumberMarker(Integer.valueOf(i10)));
    ...
    private String initiateMultipartUpload(PutObjectRequest putObjectRequest, boolean z10) {
        ...
        String uploadId = this.f٦٩١٩s3.initiateMultipartUpload(withObjectMetadata).getUploadId();
    ...
    private UploadResult uploadInOneChunk() {
        PutObjectResult putObject = this.f٦٩١٩s3.putObject(this.putObjectRequest);
    ...
    private UploadResult uploadInParts() throws Exception {
        boolean z10 = this.f٦٩١٩s3 instanceof AmazonS3EncryptionClient;
        long optimalPartSize = getOptimalPartSize(z10);
        if (this.multipartUploadId == null) {
            this.multipartUploadId = initiateMultipartUpload(this.putObjectRequest, z10);
        }
        try {
            try {
                ...
                if (TransferManagerUtils.isUploadParallelizable(this.putObjectRequest, z10)) {
            ...
            if (this.putObjectRequest.getInputStream() != null) {
                try {
                    ...
                    log.warn("Unable to cleanly close input stream: " + e12.getMessage(), e12);
                }
            }
        }
    }
        ...
        while (uploadPartRequestFactory.hasMoreRequests()) {
            if (!this.threadPool.isShutdown()) {
                ...
                if (identifyExistingPartsForResume.containsKey(Integer.valueOf(nextUploadPartRequest.getPartNumber()))) {
                    ...
                    this.futures.add(this.threadPool.submit(new UploadPartCallable(this.f٦٩١٩s3, nextUploadPartRequest)));
                }
            } else {
    ...
    private UploadResult uploadPartsInSeries(UploadPartRequestFactory uploadPartRequestFactory) {
        ...
        while (uploadPartRequestFactory.hasMoreRequests()) {
            if (!this.threadPool.isShutdown()) {
                ...
                arrayList.add(this.f٦٩١٩s3.uploadPart(nextUploadPartRequest).getPartETag());
            } else {
                throw new CancellationException("TransferManager has been shutdown");
            }
        }
        CompleteMultipartUploadResult completeMultipartUpload = this.f٦٩١٩s3.completeMultipartUpload(new CompleteMultipartUploadRequest(this.putObjectRequest.getBucketName(), this.putObjectRequest.getKey(), this.multipartUploadId, arrayList));
        UploadResult uploadResult = new UploadResult();
    ...
    public void performAbortMultipartUpload() {
        try {
            if (this.multipartUploadId != null) {
                this.f٦٩١٩s3.abortMultipartUpload(new AbortMultipartUploadRequest(this.putObjectRequest.getBucketName(), this.putObjectRequest.getKey(), this.multipartUploadId));
            }
        } catch (Exception e10) {
            log.info("Unable to abort multipart upload, you may need to manually remove uploaded parts: " + e10.getMessage(), e10);
        }
    }
