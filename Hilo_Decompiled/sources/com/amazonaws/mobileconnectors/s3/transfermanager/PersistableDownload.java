...
public final class PersistableDownload extends PersistableTransfer {
    ...
    public String serialize() {
        ...
        try {
            jsonWriter.beginObject().name("pauseType").value(TYPE).name("bucketName").value(this.bucketName).name(TransferTable.COLUMN_KEY).value(this.key).name("file").value(this.file).name("versionId").value(this.versionId).name("isRequesterPays").value(this.isRequesterPays);
            if (this.range != null) {
