...
public class DownloadDispatcher {
    ...
    public static void setMaxParallelRunningCount(int i10) {
        ...
        if (downloadDispatcher.getClass() == DownloadDispatcher.class) {
            downloadDispatcher.maxParallelRunningCount = Math.max(1, i10);
            return;
        }
    ...
    public synchronized void finish(DownloadCall downloadCall) {
        ...
        try {
            boolean z10 = downloadCall.asyncExecuted;
            ...
            if (list.remove(downloadCall)) {
                if (z10 && downloadCall.isCanceled()) {
    ...
    public boolean inspectCompleted(DownloadTask downloadTask, Collection<DownloadTask> collection) {
    ...
    public boolean cancel(int i10) {
        this.skipProceedCallCount.incrementAndGet();
        boolean cancelLocked = cancelLocked(DownloadTask.mockTaskForCompare(i10));
        this.skipProceedCallCount.decrementAndGet();
        processCalls();
        return cancelLocked;
    }
