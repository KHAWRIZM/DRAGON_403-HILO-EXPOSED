...
public final class DebugProbesImpl {
    ...
    private final <T> Continuation<T> createOwner(Continuation<? super T> completion, StackTraceFrame frame) {
        ...
        CoroutineOwner<?> coroutineOwner = new CoroutineOwner<>(completion, new DebugCoroutineInfo(context, frame, atomicLongFieldUpdater.incrementAndGet(debugProbesImpl$VolatileWrapper$atomicfu$private)));
    ...
    private final void dumpCoroutinesSynchronized(PrintStream out) {
        ...
        if (isInstalled$kotlinx_coroutines_debug()) {
            ...
            for (CoroutineOwner coroutineOwner : SequencesKt.sortedWith(SequencesKt.filter(CollectionsKt.asSequence(getCapturedCoroutines()), new Function1<CoroutineOwner<?>, Boolean>() { // from class: kotlinx.coroutines.debug.internal.DebugProbesImpl$dumpCoroutinesSynchronized$2
                ...
                List<StackTraceElement> enhanceStackTraceWithThreadDumpImpl = debugProbesImpl.enhanceStackTraceWithThreadDumpImpl(debugCoroutineInfo.get_state(), debugCoroutineInfo.lastObservedThread, lastObservedStackTrace$kotlinx_coroutines_core);
                if (Intrinsics.areEqual(debugCoroutineInfo.get_state(), DebugCoroutineInfoImplKt.RUNNING) && enhanceStackTraceWithThreadDumpImpl == lastObservedStackTrace$kotlinx_coroutines_core) {
                    str = debugCoroutineInfo.get_state() + " (Last suspension stacktrace, not an actual stacktrace)";
                } else {
    ...
    private final List<StackTraceElement> enhanceStackTraceWithThreadDumpImpl(String state, Thread thread, List<StackTraceElement> coroutineTrace) {
        ...
        if (Intrinsics.areEqual(state, DebugCoroutineInfoImplKt.RUNNING) && thread != null) {
            try {
                ...
                obj = Result.m٥٤٤constructorimpl(thread.getStackTrace());
            } catch (Throwable th) {
    ...
    private final Function1<Boolean, Unit> getDynamicAttach() {
        ...
        try {
            ...
            obj = Result.m٥٤٤constructorimpl((Function1) TypeIntrinsics.beforeCheckcastToFunctionOfArity(newInstance, 1));
        } catch (Throwable th) {
            Result.Companion companion2 = Result.INSTANCE;
            obj = Result.m٥٤٤constructorimpl(ResultKt.createFailure(th));
        }
    ...
    private final void updateRunningState(CoroutineStackFrame frame, String state) {
        ...
        remove.updateState$kotlinx_coroutines_core(state, (Continuation) frame, z10);
