...
public final class UShortArrayBuilder extends PrimitiveArrayBuilder<UShortArray> {
    ...
    public final void m٢٨٥appendxj2QHRw$kotlinx_serialization_core(short c) {
        PrimitiveArrayBuilder.ensureCapacity$kotlinx_serialization_core$default(this, 0, 1, null);
        short[] sArr = this.buffer;
        int position = getPosition();
        this.position = position + 1;
        UShortArray.set-01HTLdE(sArr, position, c);
    }

    @Override // kotlinx.serialization.internal.PrimitiveArrayBuilder
    public /* bridge */ /* synthetic */ UShortArray build$kotlinx_serialization_core() {
        return UShortArray.box-impl(m٢٨٦buildamswpOA$kotlinx_serialization_core());
    }

    @NotNull
    /* renamed from: build-amswpOA$kotlinx_serialization_core, reason: not valid java name */
    public short[] m٢٨٦buildamswpOA$kotlinx_serialization_core() {
        short[] copyOf = Arrays.copyOf(this.buffer, getPosition());
        Intrinsics.checkNotNullExpressionValue(copyOf, "copyOf(...)");
        return UShortArray.constructor-impl(copyOf);
    }
    ...
    public void ensureCapacity$kotlinx_serialization_core(int requiredCapacity) {
        if (UShortArray.getSize-impl(this.buffer) < requiredCapacity) {
            short[] sArr = this.buffer;
            short[] copyOf = Arrays.copyOf(sArr, RangesKt.coerceAtLeast(requiredCapacity, UShortArray.getSize-impl(sArr) * 2));
            Intrinsics.checkNotNullExpressionValue(copyOf, "copyOf(...)");
            this.buffer = UShortArray.constructor-impl(copyOf);
        }
    }
    ...
    private UShortArrayBuilder(short[] sArr) {
        ...
        this.buffer = sArr;
        this.position = UShortArray.getSize-impl(sArr);
