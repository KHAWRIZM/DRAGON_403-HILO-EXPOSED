...
public final class Response<T> {
    ...
    public static <T> Response<T> error(int i, ResponseBody responseBody) {
        ...
        if (i >= 400) {
            return error(responseBody, new Response.Builder().body(new OkHttpCall.NoContentResponseBody(responseBody.get$contentType(), responseBody.getContentLength())).code(i).message("Response.error()").protocol(Protocol.HTTP_1_1).request(new Request.Builder().url("http://localhost/").build()).build());
        }
