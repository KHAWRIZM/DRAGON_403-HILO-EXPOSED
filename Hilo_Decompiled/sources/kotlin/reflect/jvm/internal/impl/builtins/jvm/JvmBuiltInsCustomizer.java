...
public final class JvmBuiltInsCustomizer implements AdditionalClassPartsProvider, PlatformDependentDeclarationFilter {
    ...
    public static final Iterable getJdkMethodStatus$lambda$16(JvmBuiltInsCustomizer jvmBuiltInsCustomizer, ClassDescriptor classDescriptor) {
        ...
        while (it.hasNext()) {
            ClassifierDescriptor classifierDescriptor2 = ((KotlinType) it.next()).getConstructor().mo١٩٢٣getDeclarationDescriptor();
            LazyJavaClassDescriptor lazyJavaClassDescriptor = null;
            if (classifierDescriptor2 != null) {
                classifierDescriptor = classifierDescriptor2.getOriginal();
            } else {
    ...
    private final boolean isTrivialCopyConstructorFor(ConstructorDescriptor constructorDescriptor, ClassDescriptor classDescriptor) {
        ...
        if (constructorDescriptor.getValueParameters().size() == 1) {
            ...
            ClassifierDescriptor classifierDescriptor = ((ValueParameterDescriptor) CollectionsKt.single((List) valueParameters)).getType().getConstructor().mo١٩٢٣getDeclarationDescriptor();
            if (classifierDescriptor != null) {
                fqNameUnsafe = DescriptorUtilsKt.getFqNameUnsafe(classifierDescriptor);
            } else {
                fqNameUnsafe = null;
            }
    ...
    */
    public Collection<SimpleFunctionDescriptor> getFunctions(@NotNull final Name name, @NotNull ClassDescriptor classDescriptor) {
        ...
        for (SimpleFunctionDescriptor simpleFunctionDescriptor : additionalFunctions) {
            ...
            newCopyBuilder.setPreserveSourceElement();
            int i10 = WhenMappings.$EnumSwitchMapping$0[getJdkMethodStatus(simpleFunctionDescriptor).ordinal()];
