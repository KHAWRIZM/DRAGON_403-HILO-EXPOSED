...
public final class KPropertyImplKt {
    ...
    */
    public static final Caller<?> computeCallerForAccessor(KPropertyImpl.Accessor<?, ?> accessor, boolean z10) {
        ...
        if (mapPropertySignature instanceof JvmPropertySignature.KotlinProperty) {
                    ...
                    if (method != null) {
                        if (InlineClassesUtilsKt.isUnderlyingPropertyOfInlineClass(accessor.getProperty().getDescriptor()) && Intrinsics.areEqual(accessor.getProperty().getDescriptor().getVisibility(), DescriptorVisibilities.INTERNAL)) {
                            ...
                            if (inlineClass != null && (inlineClassUnboxMethod = ValueClassAwareCallerKt.getInlineClassUnboxMethod(inlineClass, accessor.getProperty().getDescriptor())) != null) {
                                if (accessor.isBound()) {
                                    instance = new InternalUnderlyingValOfInlineClass.Bound(inlineClassUnboxMethod, getBoundReceiver(accessor));
                                } else {
                                    instance = new InternalUnderlyingValOfInlineClass.Unbound(inlineClassUnboxMethod);
                                }
                            } else {
                            ...
                            if (javaField != null) {
                                instance = computeCallerForAccessor$computeFieldCaller(accessor, z10, javaField);
                            } else {
                        ...
                        if (!Modifier.isStatic(method.getModifiers())) {
                            ...
                            caller = new CallerImpl.Method.BoundStatic(method, false, getBoundReceiver(accessor));
                        } else {
            ...
            instance = computeCallerForAccessor$computeFieldCaller(accessor, z10, ((JvmPropertySignature.JavaField) mapPropertySignature).getField());
        ...
        return ValueClassAwareCallerKt.createValueClassAwareCallerIfNeeded$default(instance, accessor.getDescriptor(), false, 2, null);
    }
